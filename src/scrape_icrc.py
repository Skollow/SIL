...
def run_scrape_icrc() -> int:
    init_db()
    new_total = 0

    for page in range(1, MAX_PAGES + 1):
        url = PAGE_1_URL if page == 1 else PAGED_URL.format(page)
        print(f"[ICRC] Scraping page {page}: {url}")

        try:
            res = requests.get(url, headers=HEADERS, timeout=15)
        except requests.RequestException as e:
            print(f"[ICRC] Request error: {e}")
            break

        if res.status_code == 404:
            print("[ICRC] No more pages.")
            break
        if res.status_code != 200:
            print(f"[ICRC] Status {res.status_code}, stopping.")
            break

        soup      = BeautifulSoup(res.text, "html.parser")
        link_tags = []
        for selector in [
            "article.post-short a",
            "h2.entry-title a",
            "h3.entry-title a",
            "h2.post-title a",
            "h3.post-title a",
        ...
        if not link_tags:
            print("[ICRC] No links found, stopping.")
    ...
    print(f"[ICRC] Done — {new_total} new articles total.")
    return new_total