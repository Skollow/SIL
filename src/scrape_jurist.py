...
def parse_date(date_text):
    ...
    for fmt in ["%B %d, %Y", "%b %d, %Y", "%d %B %Y", "%Y-%m-%d", "%b. %d, %Y"]:
        try:
...
def scrape_article(url):
    ...
    title_tag = (
        soup.select_one("strong.ui-heading3") or
...
def run_scrape_jurist() -> int:
    init_db()
    new_total = 0

    for page in range(1, MAX_PAGES + 1):
        url = PAGE_1_URL if page == 1 else CATEGORY_URL.format(page)
        print(f"[JURIST] Scraping page {page}: {url}")

        try:
            res = requests.get(url, headers=HEADERS, timeout=15)
        except requests.RequestException as e:
            print(f"[JURIST] Request error: {e}")
            break

        if res.status_code == 404:
            print("[JURIST] No more pages.")
            break
        if res.status_code != 200:
            print(f"[JURIST] Status {res.status_code}, stopping.")
            break
        ...
        if not link_tags:
            print("[JURIST] No links found, stopping.")
        ...
        for a in link_tags:
            ...
            if not link.startswith(BASE_URL):
                continue
            if any(x in link for x in ["/author/", "/category/", "/tag/", "/page/"]):
                continue
    ...
    print(f"[JURIST] Done — {new_total} new articles total.")
    return new_total